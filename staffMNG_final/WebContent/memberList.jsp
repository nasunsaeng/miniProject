...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$(".mytr").click(function(){
			...
			var param={"empno":empno,"flag":"list"};
			$.ajax({
				url:"${path}/memberDetail.fo",
				type: "post",
				data: param,
...
// 			e.preventDefault();	기본이벤트제거
		...
		$('#excel_btn').click(function(){
			...
			var exceltypes =$('input:radio[name="exceltype"]:checked'). val() ;
			...
			alert("elements = "+elements +"<br/>"+ "exceltypes ="+exceltypes);
	...
	function doDelete(){
		...
		if( $('input[name="chkDel"]:checked').length == 0){
 		...
 		if(confirm("선택한 항목을 삭제하시겠습니까?")){
			var param={"empno":empno,"curPage":curPage,"flag":"list"};
			$.ajax({
				url:"${path}/memberDelete.fo",
				type: "post",
				data: param,
				success: function(result){
					$("#emplist").html(result);
